...
namespace MeshCombineStudio
{
    public class MCSProducts
    {
        ...
        void UpdateSale()
        {
            ...
            if (waitSecondsForRecheck < 10)
                waitSecondsForRecheck = 10;

            showAs = lines[2].Contains("AS__");
            if (showAs)
                asUrl = lines[2].Substring(5);
            ...
            if (deSale)
                deSaleText = "50% Discount on Unity's " + saleText + " NOW!\n" + lines[3].Replace("DE_1 ", "") + "\n\n";
            if (mcsSale)
                mcsSaleText = "50% Discount on Unity's " + saleText + " NOW!\n" + lines[4].Replace("MCS_1 ", "") + "\n\n";
            if (mcsCavesSale)
                mcsCavesSaleText = "50% Discount on Unity's " + saleText + " NOW!\n" + lines[5].Replace("MCSCaves_1 ", "") + "\n\n";
            if (wcSale)
                wcSaleText = "50% Discount on Unity's " + saleText + " NOW!\n" + lines[6].Replace("WC_1 ", "") + "\n\n";
            if (tcSale)
                tcSaleText = "50% Discount on Unity's " + saleText + " NOW!\n" + lines[7].Replace("TC_1 ", "") + "\n\n";
        ...
        void GetPlayerPrefs()
        {
            saleText = PlayerPrefs.GetString("PON_sale");
            hasSale = ( saleText != string.Empty );
            deSaleText = PlayerPrefs.GetString("PON_deSale");
            deSale = ( deSaleText != string.Empty );
            mcsSaleText = PlayerPrefs.GetString("PON_mcsSale");
            mcsSale = ( mcsSaleText != string.Empty );
            mcsCavesSaleText = PlayerPrefs.GetString("PON_mcsCavesSale");
            mcsCavesSale = ( mcsCavesSaleText != string.Empty );
            tcSaleText = PlayerPrefs.GetString("PON_tcSale");
            tcSale = ( tcSaleText != string.Empty );
            wcSaleText = PlayerPrefs.GetString("PON_wcSale");
            wcSale = ( wcSaleText != string.Empty );
            asUrl = PlayerPrefs.GetString("PON_asUrl");
            showAs = ( asUrl != string.Empty );
        }
        ...
        public void Draw(MonoBehaviour monoBehaviour)
        {
            ...
            if (!mcsInProject)
                DrawMCS(ref rect, width, alpha);
            if (!mcsCavesInProject)
                DrawMCSCaves(ref rect, width, alpha);
            if (!deInProject)
                DrawDE(ref rect, width, alpha);
            if (!tcInProject)
                DrawTC(ref rect, width, alpha);
            if (!wcInProject)
                DrawWC(ref rect, width, alpha);
