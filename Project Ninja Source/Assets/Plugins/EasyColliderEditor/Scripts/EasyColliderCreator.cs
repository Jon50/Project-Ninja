...
namespace ECE
{
  ...
  public class EasyColliderCreator
  {
    ...
    public List<EasyColliderData> CalculateRotateAndDuplicate(CREATE_COLLIDER_TYPE result, List<Vector3> worldVertices, Transform attachTo, EasyColliderRotateDuplicate ecrd)
    {
      // Debug.Log(result);
      ...
      switch (result)
      {
        ...
        case CREATE_COLLIDER_TYPE.ROTATED_BOX:
          baseData = CalculateBox(worldVertices, attachTo, true);
          break;
        ...
        case CREATE_COLLIDER_TYPE.ROTATED_CAPSULE:
          baseData = CalculateCapsuleMinMax(worldVertices, attachTo, ECEPreferences.CapsuleColliderMethod, true);
          break;
      ...
      axis = ecrd.axis == EasyColliderRotateDuplicate.ROTATE_AXIS.X ? attachTo.right : axis;
      axis = ecrd.axis == EasyColliderRotateDuplicate.ROTATE_AXIS.Y ? attachTo.up : axis;
      axis = ecrd.axis == EasyColliderRotateDuplicate.ROTATE_AXIS.Z ? attachTo.forward : axis;
      // Debug.DrawLine(attachTo.position, attachTo.position + axis, Color.yellow);
      for (int i = 0; i < ecrd.NumberOfDuplications; i++)
      {
        Quaternion q = Quaternion.AngleAxis(currentAngle, axis);
        Matrix4x4 m = Matrix4x4.TRS(attachTo.position, q * baseData.Matrix.rotation, baseData.Matrix.lossyScale);
        //TODO: COLLIDER DUPLICATION IS NOT CREATING THE CORRECT NUMBER OF COLLIDERS / THEY ARE OVERLAPPING
        switch (result)
        {
          case CREATE_COLLIDER_TYPE.CAPSULE:
          case CREATE_COLLIDER_TYPE.ROTATED_CAPSULE:
    ...
    public List<Collider> CreateRotateAndDuplicateColliders(CREATE_COLLIDER_TYPE collider_type, List<Vector3> worldVertices, EasyColliderProperties properties)
    {
      ...
      List<EasyColliderData> data = CalculateRotateAndDuplicate(collider_type, worldVertices, ecrd.pivot.transform, ecrd);
      foreach (EasyColliderData d in data)
      {
...
#endif
        obj.transform.rotation = d.Matrix.rotation;
        ...
        switch (collider_type)
        {
          case CREATE_COLLIDER_TYPE.BOX:
          case CREATE_COLLIDER_TYPE.ROTATED_BOX:
            collider = CreateBoxCollider(d as BoxColliderData, properties);
            break;
          case CREATE_COLLIDER_TYPE.SPHERE:
            // ECEditor.CreateSphereCollider(ECEPreferences.SphereColliderMethod);
            collider = CreateSphereCollider(d as SphereColliderData, properties);
            break;
          case CREATE_COLLIDER_TYPE.CAPSULE:
          case CREATE_COLLIDER_TYPE.ROTATED_CAPSULE:
            collider = CreateCapsuleCollider(d as CapsuleColliderData, properties);
            break;
          case CREATE_COLLIDER_TYPE.CONVEX_MESH:
          case CREATE_COLLIDER_TYPE.CYLINDER:
            collider = CreateConvexMeshCollider((d as MeshColliderData).ConvexMesh, properties.AttachTo, properties);
            break;
        }
