...
namespace ECE
{
  ...
  public class EasyColliderPreviewer : ScriptableObject
  {
    ...
    public List<EasyColliderData> RotateAndDuplicateData;
    ...
    private void DrawPreview()
    {
      ...
      else
      {
        ...
        {
          foreach (EasyColliderData PreviewData in RotateAndDuplicateData)
          {
    ...
    private void CalculatePreviewRotateAndDuplicate(CREATE_COLLIDER_TYPE type, List<Vector3> worldVertices, GameObject attachTo, int method, EasyColliderRotateDuplicate ecrd)
    {
      ...
      CurrentVertexCount = worldVertices.Count;
      CurrentAttachTo = attachTo.transform;
      if (worldVertices.Count >= GetMinVerticesForColliderType(type))
      {
        EasyColliderCreator ecc = new EasyColliderCreator();
        RotateAndDuplicateData = ecc.CalculateRotateAndDuplicate(type, worldVertices, attachTo.transform, ecrd);
      }
    }
    ...
    private bool NeedsUpdate(EasyColliderEditor editor, CREATE_COLLIDER_TYPE colliderType, int method)
    {
      if (CurrentVertexCount != editor.SelectedVertices.Count
        || (PreviewData == null && RotateAndDuplicateData == null)
        || (PreviewData != null && PreviewData.ColliderType != colliderType)
        || CurrentMethod != method
    ...
    public void UpdatePreview(EasyColliderEditor editor, EasyColliderPreferences preferences, bool forceUpdate = false)
    {
      if (editor != null && preferences != null && editor.SelectedGameObject != null && editor.AttachToObject != null)
      {
        ...
        if (NeedsUpdate(editor, preferences.PreviewColliderType, GetMethodForColliderPreviewType(preferences)) || forceUpdate)
        {
          ...
          {
            CalculatePreviewRotateAndDuplicate(preferences.PreviewColliderType, editor.GetWorldVertices(), preferences.rotatedDupeSettings.pivot, GetMethodForColliderPreviewType(preferences), preferences.rotatedDupeSettings);
          }
          else
          {
