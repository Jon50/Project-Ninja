    ...
    public class CsvImporterTool : TongueTwisterTool
    {
        ...
        public CsvImporterTool()
        {
            ...
            if (EditorPrefs.HasKey(LAST_USED_CSV_IMPORTER_SETTINGS))
            {
                ...
                if (_csvImporterSettings) CreateCsvImporterSettingsEditor();
            }
        }
        ...
        public override void DrawEditorUi()
        {
            ...
            if (fileType == CsvFileType.None) return;
            ...
            if (localeImportType == LocaleImportType.None) return;
        ...
        private void Import()
        {
            try
            {
                if (_debug) Debug.Log($"{DEBUG_TAG}Starting CSV import");

                ValidateCsvImporterSettings();
                
                if (_debug) Debug.Log($"{DEBUG_TAG}Validated CSV Importer Settings");
                ...
                if (_debug) Debug.Log($"{DEBUG_TAG}Found {dataRows.Count} rows.");

                var columns = dataRows[0];
                dataRows.RemoveAt(0);

                if (_debug) Debug.Log($"{DEBUG_TAG}Found {columns.Count} columns.");
                ...
                if (csvFileType == CsvFileType.LocaleColumns)
                {
                    ...
                    for (int column = 1; column < columns.Count; column++)
                    {
                        ...
                        if (_debug) Debug.Log($"{DEBUG_TAG}Evaluating column named \"{columnName}\"");
                        ...
                        for (int row = 0; row < dataRows.Count; row++)
                        {
                            ...
                            if (_debug) Debug.Log($"{DEBUG_TAG}Imported \"{displayKey}\":\"{translation}\"");
                        }
                    }
                }
                else if (csvFileType == CsvFileType.DisplayKeyColumns)
                {
                    ...
                    for (int column = 1; column < columns.Count; column++)
                    {
                        ...
                        if (_debug) Debug.Log($"{DEBUG_TAG}Evaluating column named \"{displayKey}\"");
        ...
        private string GetDisplayKeyForCsvPreview(int displayKey)
        {
            ...
            switch (displayKey)
            {
                case 0: return $"\"Greetings{groupDelimiter}Greetings1\",";
                case 1: return "\"Greetings2\",";
                case 2: return $"\"Departures{groupDelimiter}Byes{groupDelimiter}Bye1\",";
                default: return null;
            }
        }
        ...
        private string GetLocaleForCsvPreview(int locale, LocaleImportType localeImportType)
        {
            switch (locale)
            {
                case 0: return localeImportType == LocaleImportType.LocalizationCode ? "\"en-us\"," : "\"American English\",";
                case 1: return localeImportType == LocaleImportType.LocalizationCode ? "\"es-mx\"," : "\"Mexican Spanish\",";
                case 2: return localeImportType == LocaleImportType.LocalizationCode ? "\"de-de\"," : "\"German\",";
                default: return null;
            }
        }
        ...
        private static string GetDelimiter(CsvImporterSettings csvImporterSettings)
        {
            ...
            {
                case DelimiterModeType.Comma: return ",";
                case DelimiterModeType.Space: return " ";
                case DelimiterModeType.Tab: return "\t";
                case DelimiterModeType.Custom: return csvImporterSettings.customCsvDelimiter;
                default: return "";
            }
        }
    }
