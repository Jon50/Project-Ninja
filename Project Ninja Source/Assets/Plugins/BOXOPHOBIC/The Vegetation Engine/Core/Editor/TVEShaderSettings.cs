...
namespace TheVegetationEngine
{
    public class TVEShaderSettings : EditorWindow
    {
        ...
        void OnEnable()
        {
            ...
            var cgincQR = "Assets/Mega World/Quadro Renderer/Shaders/Include/QuadroRendererInstancerInclude.cginc";

            searchFolders = AssetDatabase.FindAssets("QuadroRendererInstancerInclude");

            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("QuadroRendererInstancerInclude.cginc"))
                {
        ...
        void InjectShaderFeature(string shaderAssetPath, string renderEngine)
        {
            ...
            else if (renderEngine.Contains("Mega World Quadro Renderer"))
            {
                for (int i = 0; i < lines.Count; i++)
                {
                    if (lines[i].Contains("SHADER INJECTION POINT BEGIN"))
