...
namespace TheVegetationEngine
{
    public class TVEHub : EditorWindow
    {
        ...
        bool requireMaterialsUpgrade = false;
        ...
        void OnEnable()
        {
            ...
            corePackagesPath = assetFolder + "/Core/Pipelines";
        ...
        void OnGUI()
        {
            ...
            else
            {
                ...
                {
                    EditorGUILayout.HelpBox("Click the Render Pipeline Import button to install the shaders for the choosed render pipeline! Starting with version 2.2.0+, the shader settings are moved to Window > BOXOPHOBIC > The Vegetation Engine > Shader Settings!", MessageType.Info, true);
                }
                ...
                if (requireMaterialsUpgrade)
                {
                    if (GUILayout.Button("Finish High Definition Pipeline Setup", GUILayout.Height(GUI_HEIGHT)))
                    {
                        ...
                        requireMaterialsUpgrade = false;

#if !UNITY_2020_2_OR_NEWER
                        if (corePackageOptions[corePackageIndex].Contains("High"))
                        {
        ...
        void ImportPackage()
        {
            ...
            Debug.Log("[The Vegetation Engine Installer] " + corePackageOptions[corePackageIndex] + " package imported!");
        }
            ...
            if (shaderAssetPath.Contains("Subsurface"))
            {
                ...
                if (shaderAssetPath.Contains("Subsurface"))
                {
                    for (int j = 0; j < lines.Count; j++)
                    {
                        ...
                        {
                            lines[j] = lines[j].Replace("[HideInInspector][ASEDiffusionProfile(_SubsurfaceDiffusion)]", "[ASEDiffusionProfile(_SubsurfaceDiffusion)]");
                        }
