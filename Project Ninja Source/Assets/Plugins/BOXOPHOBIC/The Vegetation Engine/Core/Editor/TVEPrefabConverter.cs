...
namespace TheVegetationEngine
{
    public class TVEPrefabConverter : EditorWindow
    {
...
#else
        const int SQUARE_BUTTON_WIDTH = 16;
        const int SQUARE_BUTTON_HEIGHT = 14;
#endif
        ...
        "[8]  Bottom To Top", "[9]  Top To Bottom", "[10]  Bottom Projection", "[11]  Top Projection", "[12]  Height Exp", "[13]  Hemi Sphere", "[14]  Hemi Cylinder", "[16]  Hemi Capsule"
        };
        ...
        void DrawConvert()
        {
            ...
            if (GUILayout.Button("Convert"))
            {
                if (convertedPrefabCount > 0)
                {
                    keepMaterials = EditorUtility.DisplayDialog("Reconvert Materials?", "Choose if the original materials should be reconverted or keep the current converted materials.", "Keep", "Reconvert Materials");
        ...
        //{
        //    index = StyledPopup(name, "", index, options);
        ...
        {
            var saveFullPath = prefabDataFolder + "/" + mesh.name.Replace(":", "") + " (" + outputSuffix + " Mesh).mesh";
        ...
        void GetMaterialConversionFromPreset(Material materialOriginal, Material materialInstance)
        {
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                ...
                if (presetLines[i].StartsWith("Utility") && useLine)
                {
                    ...
                    //if (type == "START_TEXTURE_PACKING_AND_DELETE_UNUSED")
                    //{
                    //    doPacking = true;
                    //    doCleanup = true;
                    //}
                ...
                if (doPacking)
                {
                    ...
                    if (maskTextures[0] != null || maskTextures[1] != null || maskTextures[2] != null || maskTextures[3] != null)
                    {
                        ...
                        for (int n = 0; n < packedTextureNames.Count; n++)
                        {
                            ...
                            {
                                materialInstance.SetTexture(propName, packedTextureObjcts[n]);
                                exist = true;
                            }
                        }
                        ...
                        if (exist == false)
                        {
                            PackTexture(materialInstance, id, internalName, propName);
                        }
                    }
                    ...
                    //        {
                    //            var assetPath = AssetDatabase.GetAssetPath(maskTextures[t]);

                    //            if (assetPath.Contains(prefabDataFolder))
                    //            {
            ...
            for (int i = 0; i < materialArraysInPrefab.Count; i++)
            {
                if (materialArraysInPrefab[i] != null)
                {
                    for (int j = 0; j < materialArraysInPrefab[i].Length; j++)
                    {
                        ...
                        if (keepMaterials && File.Exists(materialPath))
                        {
        ...
        {
            var saveFullPath = prefabDataFolder + "/" + materialInstance.name + " (" + outputSuffix + " Material).mat";
        ...
        void PackTexture(Material materialInstance, int id, string internalName, string propName)
        {
            ...
            string texPath = prefabDataFolder + "/" + saveName + " " + id.ToString() + " (" + outputSuffix + " Texture).png";
        ...
        {
            shaderCross = Shader.Find("BOXOPHOBIC/The Vegetation Engine/Vegetation/Cross Standard Lit (Subsurface)");
            shaderLeaf = Shader.Find("BOXOPHOBIC/The Vegetation Engine/Vegetation/Leaf Standard Lit (Subsurface)");
            shaderBark = Shader.Find("BOXOPHOBIC/The Vegetation Engine/Vegetation/Bark Standard Lit (Deferred)");
            shaderGrass = Shader.Find("BOXOPHOBIC/The Vegetation Engine/Vegetation/Grass Standard Lit (Subsurface)");
            shaderProp = Shader.Find("BOXOPHOBIC/The Vegetation Engine/Objects/Prop Standard Lit (Deferred)");
        }
        ...
        void GetPresets()
        {
            ...
            {
                OverridesEnum[i] = AssetDatabase.LoadAssetAtPath<Object>(overridePaths[i]).name;
